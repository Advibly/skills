...
The signature editorial look is usually halftone print collage on a flat bold color field, with clean machine-cut edges and a soft drop shadow lifting each piece.
...
The flat field is what makes the cut-outs pop. Get the hex right; generators drift on saturated fields. This is the field you keep faithful (`on_brand: false`) unless the user explicitly asks to recolor to their brand.
...
State each element's shadow separately when layers float at different heights. The layer-by-layer float is also what the assemble-from-empty motion animates: each piece slides in and its shadow settles as it lands.
